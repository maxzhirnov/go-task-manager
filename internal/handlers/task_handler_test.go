...
func TestCreateTask(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			mockSetup: func(mock sqlmock.Sqlmock) {
				// No mock needed as it should fail before DB call
			},
			expectedStatus: http.StatusBadRequest,
...
func TestUpdateTask(t *testing.T) {
	tests := []struct {
		name           string
		taskID         string
		inputTask      interface{} // interface{} to allow invalid JSON
		mockSetup      func(sqlmock.Sqlmock)
		expectedStatus int
		expectedError  string
		expectedTask   *models.Task
	}{
		{
			...
			mockSetup: func(mock sqlmock.Sqlmock) {
				mock.ExpectExec("UPDATE tasks SET title = \\$1, description = \\$2, status = \\$3, updated_at = \\$4 WHERE id = \\$5").
					WithArgs(
						"Updated Task",
						"Updated Description",
						"in_progress",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Marshal task to JSON
			var body []byte
			if str, ok := tt.inputTask.(string); ok {
				body = []byte(str)
			} else {
				body, err = json.Marshal(tt.inputTask)
				assert.NoError(t, err)
			}

			req, err := http.NewRequest("PUT", "/api/tasks/"+tt.taskID, bytes.NewBuffer(body))
			assert.NoError(t, err)
